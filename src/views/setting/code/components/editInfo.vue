...
import {getPackageApi} from "@/api/setting/package";
...
const init = () => {
  ...
  // 模块列表
  getPackageApi().then( res => modules.value = res.data.pkgs )
  // 请求菜单列表
  menuApi.tree({ onlyMenu: true }).then( res => {
    let data=[]
    for(var i=0;i<res.data.menus.length;i++){
      let childrens=res.data.menus[i].children
      let ccs=[]
      for(var j=0;j<childrens.length;j++){
       ...
       let item1={
         label:res.data.menus[i].name,
         value:res.data.menus[i].ID,
         parent_id:res.data.menus[i].parent_id,
         children:ccs
